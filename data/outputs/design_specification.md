...
- 设计 ID：`QD-FFEC8E83-MAGNET-01`
- 输入：`D:\Users\Lenovo\Desktop\QianCraft\data\outputs\designer_handoff.json`
- 输入 SHA-256：`ffec8e836d0498cbcb1999b827dd73164f791137b0a9a11ecd39fcd30f39fdfa`
...
- 类型：可替换织物面板冰箱贴
- 概念：以原创数纱网格模块展示针脚秩序，面板可拆换并连接溯源卡。
- 形态：圆角方形框体、可拆织物面板和背部封闭磁体。
- 风格：模块化、像素化、清晰网格、克制强对比
- 色彩：深色中性框体；原型彩线关系待实物线卡确认
- 交互：更换织物面板、系列编号、溯源阅读
...
- 地域/社区：贵州省贵阳市花溪区（首版地域范围）｜花溪挑花；具体合作社区待确认
- 视觉作用：作为模块尺寸、针脚方向和重复秩序
- 转译规则：重新设计原创网格单元，不复制完整馆藏纹样。
- 权利规则：V001 为 reference_only，不作为生成图像输入。
- 禁止复制：完整背扇花块；其他地域专属工艺或纹样
- 证据：C001、C029
...
- 框体宽度：76 mm（±1 mm）
- 框体厚度：8 mm（±0.5 mm）
- 可替换织物面板高度：58 mm（±1 mm）
- 可替换织物面板宽度：58 mm（±1 mm）

## 5. BOM

| ID | 部件 | 材料/规格 | 工艺 | 装配 | 首样目标 | QC |
|---|---|---|---|---|---|---|
| B01 | 外框 | ABS或经工厂验证的替代材料；圆角注塑框 | 注塑 | 卡合B02 | 76×76×8 mm | 无毛边、变形 |
| B02 | 织物面板 | 棉质斜纹布；原创数纱网格刺绣 | 裁切+机器刺绣 | 卡入B01 | 58×58 mm | 套位与边缘完整 |
| B03 | 背板 | ABS；封闭磁体 | 注塑 | 与B01卡合/螺钉固定 | 平面度待确认 | 磁体不可外露 |
| B04 | 磁体 | 烧结钕铁硼或合规替代物；规格由吸附测试确定 | 采购件 | 封闭在B03 | 吸附目标待测 | 跌落后不得脱出 |
| B05 | 溯源标签 | 合成纸；批次与二维码 | 数字印刷 | 贴于背板 | 二维码≥15 mm | 扫码可读 |

## 6. 图案与工艺

### PAT-01｜原创数纱网格

- 原创规则：只使用网格、镜像和平移规则，重新设计非传统单元。
- 几何：经纬网格、十字针单元、镜像重复
- 位置：B02正面
- 工艺：机器刺绣首样
- 文化边界：不复制V001完整图案；不得与其他支系混标

## 7. 装配顺序

1. 确认框体、背板和织物面板纸样，校核圆角、缝份与磁体封闭空间。
2. 注塑/机加工B01与B03首样，去毛边并检验卡合间隙和平面度。
3. 完成B02原创网格机器刺绣、裁切和包边，检验套位后装入框体。
4. 按安全方案把B04磁体完全封闭在B03内，不使用外露磁片。
5. 卡合或可逆固定B01/B03，验证面板可拆换且无锐边、夹手点。
6. 粘贴B05并录入批次、地域、实际工艺、参与者和护理信息。
...
- Request ID：`PR-QD-FFEC8E83-MAGNET-01`
- 画布：1800×2400px
...
证据编号：C001、C002、C003、C005、C006、M007、C029
