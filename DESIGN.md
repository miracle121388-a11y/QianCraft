...
QianCraft 0.11.0 的主界面是**结果优先的生产工具**。首屏先回答四个问题：
...
- 当前版本的模型设计效果图与模型生产沟通图；历史本地结构稿必须明确标记；
- 设计名称、融合文稿、目标人群和场景；
- 采用的文化内容及原始来源；
- 采用的产品形态及代表市场记录；
- 五项评分和公开公式；
- 批次、时间、provider、模型、生成模式、完整提示词、图生图输入 SHA-256 与两张输出 SHA-256；
- 生产前验证边界；
- 进入编辑器的唯一主要入口。
...
4. 设计稿：选择配色、查看当前模型设计效果图和生成方式。
5. 生产沟通与验证：查看以设计效果图为参考生成的拆解视觉和真实待办，不能自动标为量产完成。

保存会调用模型生成两张新图并产生 V2+，右侧持续显示当前结果，避免用户在流程中失去目标。任一模型调用失败时不保存新版本，也不显示占位图。
...
- Studio 使用文档级纵向滚动，长库页、设计详情和运行中心必须能滚动到首屏以下；只有 `/workflow` 的空间画布保留固定视口与内部滚动。
...
- 0.11.0 新模型图按实际 1:1 比例展示，桌面卡片使用 `object-fit: cover` 但详情页必须提供完整图与下载；历史 3:2 结构稿保持原比例。
- 左上角显示每日排名或“手动”，右上角显示版本。
- “模型产物”只在两张文件均存在、记录 `imageGenerationUsed=true` 且包含 provider/model/mode 时显示；历史结构稿只能标为“历史结构稿”。
- 设计详情必须同时显示设计效果图与生产沟通图、两次提示词、生成方式和图生图输入摘要。
- 两个下载链接都指向当前版本的真实文件；各自 SHA-256 与下载字节一致。
...
- 新 Studio 只在两次真实图像模型调用全部成功后保存结果；未配置 provider 或任一步失败时不得显示成功、占位图或假 URL。
...
- 当前 Zeabur 图像 provider 已真实可用；新 Studio 必须逐版本显示实际模型谱系。0.10.0 历史本地结构稿继续准确标注，不能借用当前 provider 状态改写其生成历史。
