...
function DesignsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [designPayload, overview] = await Promise.all([
        listStudioDesigns(),
        getStudioOverview(),
      ]);
      setDesigns(designPayload.designs);
      setCatchingUp(
        designPayload.designs.length === 0
        && overview.today.designCount === 0
        && ['running', 'scheduled'].includes(overview.automation.dailyDesign.daily.status),
      );
      setError('');
    } catch (cause) {
      setCatchingUp(false);
  ...
  return (
    <StudioFrame view="designs" actions={<a className="studio-primary" href="/create"><Plus size={17} />新建设计</a>}>
      ...
      {designs ? <><div className="studio-tabs" role="group" aria-label="设计来源筛选">{([['all', '全部'], ['daily', '每日自动'], ['manual', '手动组合']] as const).map(([id, label]) => <button key={id} type="button" className={filter === id ? 'is-active' : ''} aria-pressed={filter === id} onClick={() => setFilter(id)}>{label}<span>{designs.filter((item) => !item.superseded && (id === 'all' || item.origin === id)).length}</span></button>)}</div><section className="studio-design-grid studio-design-grid--archive">{filtered.map((design) => <DesignCard key={design.designId} design={design} compact />)}</section>{!filtered.length ? <div className="studio-empty">{catchingUp ? <LoaderCircle className="is-spinning" size={26} /> : <PackageOpen size={26} />}<h2>{catchingUp ? '每日设计正在生成' : '这个分类还没有设计'}</h2>{catchingUp ? <p>完成后会自动刷新，无需重新打开页面。</p> : <a href="/create">创建第一个</a>}</div> : null}</> : null}
    </StudioFrame>
  );
}
          ...
          {productionAsset ? (
            <section className="studio-production-visual">
              <div>
                ...
                <h2>模型生成的生产沟通图</h2>
                <p>{productionAsset.generation?.mode === 'image_to_image' ? '它以同版本设计效果图为输入，表达部件拆解、材料方向与装配关系。' : '它使用同版本已锁定的文化事实、产品形态与设计简报再次调用生图模型，不复用旧图或占位图。'}尺寸、公差和可制造性仍须工程与工厂验证。</p>
                <dl className="studio-provenance">
                  ...
                  {productionAsset.generation?.referenceAttempt ? <div><dt>图生图尝试</dt><dd title={productionAsset.generation.referenceAttempt.detail}>{productionAsset.generation.referenceAttempt.code} · 已如实改用同方案文生图</dd></div> : null}
      ...
      {!design || !culture || !forms ? (error ? <ErrorState error={error} /> : <LoadingState label="正在载入设计工作流" />) : (
        <div className="studio-editor">
          ...
          <section className="studio-editor__panel">
            ...
            {stage === 'production' ? <><header><span>05 / PRODUCTION</span><h2>生产沟通图与生产前验证</h2><p>{design.production.asset ? (design.production.asset.generation?.mode === 'image_to_image' ? '生产沟通图由生图模型从同版本设计稿继续生成；验证事项不会被模型自动标成完成。' : '生产沟通图由生图模型使用同版本方案文稿独立生成；没有复用旧图或占位图。') : '当前历史版本没有模型生产沟通图；重新生成后才会建立双图，验证事项仍保持未完成。'}</p></header>{design.production.asset ? <div className="studio-editor-production"><img src={studioAssetUrl(design.production.asset.imageUrl)} alt={`${design.title} V${design.version} 模型生产沟通图`} /><div><strong>{design.production.asset.generation?.mode === 'image_to_image' ? '设计稿图生图' : '同方案文生图'}</strong><small>{design.production.asset.generation?.model} · {design.production.asset.width} × {design.production.asset.height} px</small>{design.production.asset.generation?.referenceAttempt ? <small>{design.production.asset.generation.referenceAttempt.code} · 图生图受阻，已记录并改用真实文生图</small> : null}<p>这张图用于讨论部件与装配方向，不提供可直接生产的尺寸、公差或认证结论。</p></div></div> : <p className="studio-inline-error">历史版本没有模型生产沟通图；保存并重新生成后才会建立双图产物。</p>}<div className="studio-production-gates">{['地域与工艺表述共同审核', '文化内容商品化授权与收益机制', '材料、结构、耐久与安全测试', '成本、工艺、公差和供应商确认', '销售地区适用法规与标签'].map((item, index) => <div key={item}><b>{String(index + 1).padStart(2, '0')}</b><span><strong>{item}</strong><small>尚无可核验完成记录</small></span><StatusPill status="not_ready" /></div>)}</div><p className="studio-boundary"><TriangleAlert size={17} />{design.production.boundary}</p></> : null}
            {error ? <p className="studio-inline-error" role="alert">{error}</p> : null}
          </section>
