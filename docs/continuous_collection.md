...
> 适用版本：0.10.0 及以后
> 范围：文化知识来源巡检、四平台市场增量采集、每日 Top 3 设计，以及三条任务通道的运行控制面。
...
- `/healthz` 由 Nginx 转发真实 `/api/health`；响应同时包含版本、采集调度器和每日设计调度器的线程/心跳。任一调度器异常时 API 返回 503，镜像 HEALTHCHECK 随之失败，交给平台重启策略恢复，不再返回静态假 200。
- `deploy/start-zeabur.sh` 监控 Tool API、Vinext 与 Nginx；任一子进程退出时容器以失败结束，交给平台重启。
- 当前本地 0.10.0 已接入两套调度器；本次线上 0.10.0 发布结果必须在实际验收后另行补写。既有云端市场通道仍因上游运行时和四平台授权缺失而 `blocked`，378 条记录保持有时间边界的历史证据。
