...
- 网络防护：当前模板关闭 Nginx 版本回显，为 API 设置 12 req/s、burst 40 的单 IP 限流，并下发 HSTS、CSP、Permissions-Policy、X-Content-Type-Options、Referrer-Policy 与 X-Frame-Options。
...
0.10.0 把默认入口改为双库驱动、结果优先的 Studio，并增加独立每日设计调度器、Top 3 批次、自由组合、设计详情与版本编辑。本文当前先记录待发布状态；只有实际部署完成并验证 `/healthz` 版本、两个调度线程、两库计数、当日设计与 PNG 后，才会追加部署 ID 并把线上版本改为 0.10.0。
...
`/app/data/runtime/workbench` 保存旧高级画布、任务书、概念版本、研究晋级产物、DesignPackage 和海报；`/app/data/runtime/tool_workspace` 保存严格研究 `job.json`、隔离 raw/derived/outputs、旧版工具设计运行，`collection/` 下的采集配置/心跳/事件/候选，以及 `studio/` 下的每日排程、事件、批次、设计版本和 PNG。文化图谱、产品形态证据和官方设计包随镜像只读发布，运行态不会覆盖证据基线；容器重启时每日调度器会在当天无产出时补跑。
