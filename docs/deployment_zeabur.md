...
0.11.0 最终部署 `6a97e30c2e33e18a8367811b` 已在同一服务运行，容器源文件摘要与提交 `b6e3257` 一致。公网 `/healthz=200`，匿名主页与 API 为 401；健康 API 返回版本 0.11.0，采集和每日设计线程均在线、心跳新鲜。认证后的真实 Chromium 在文化库长页完成 `scrollTop 0→900`，证明 Studio 文档级纵向滚动生效且控制台无错误。

Studio 产物契约现为两张真实模型图：先生成设计效果图，生产沟通图优先使用第一张图的本地 base64 参考。若参考请求明确返回 `Arrearage` 或 `UpstreamTimeout`，才按同一锁定证据、组合和生产文稿再次调用真实文生图，并把参考失败与最终 `text_to_image` 模式写入记录；其他错误继续失败关闭。2026-09-02 自动批次 `DAY-20260902-3D65E35D` 实际保存 3 个设计、6 张 1024×1024 PNG，全部内容类型与文件 SHA-256 匹配；本批三个参考请求均为 `UpstreamTimeout`，三张生产图均由 `qwen-image-3.0-pro` 独立真实文生图，没有 Pillow、本地几何、历史图片或假 URL。任一最终生成、PNG 校验或持久文件检查失败仍不写设计索引。
...
当前 Studio 完整版发布使用从 Git `91534b0` 精确抽取的隔离上下文：650 个文件、40,329,479 字节，符号链接、禁止路径、长 API Key/Authorization/私钥模式均为 0，四个关键运行文件与本地 SHA-256 一致。发布前/后快照分别为 125 文件、19,435,099/19,435,692 未压缩字节，两者都在服务器和本地 Conda 中校验通过、排除 `browser-profile/`，并已复制到仓库与 Zeabur 持久卷之外的权限受控目录。发布后 ZIP 为 4,570,472 字节，SHA-256 `2b4aa589f72f3d365abf209722827a174a69e99574fc6397764c84280f33bc10`，`containsBrowserAuthorization=false`；这是在线尽力快照，仍不等于停机一致备份或已完成恢复演练。

0.11.0 最终发布上下文为 609 个文件、39,606,937 字节；符号链接、禁止名称、私钥和长 API Key 模式均为 0。部署 `6a97e30c2e33e18a8367811b` 中 `app/studio.py` 与图像适配器 SHA-256 分别为 `39e3d201…aacd`、`7d0b0f69…e5ae`，与本地发布源完全一致。发布前在线快照为 247 文件、43,323,358 未压缩字节、10,647,726 字节 ZIP，SHA-256 `bf08f0ba…23d8`；发布后在线快照为 272 文件、54,494,826 未压缩字节、18,572,605 字节 ZIP，SHA-256 `90eab1d4…8420`。两份均在服务器与本机独立目录校验通过、排除 `browser-profile/`；它们是在线尽力快照，不等于停机一致备份或恢复演练。
