...
Studio 主链先调用图像模型生成产品设计效果图，再优先把该图作为图生图输入生成生产沟通图。若参考请求明确返回 `Arrearage` 或 `UpstreamTimeout`，系统按同一锁定证据、组合与生产文稿改发一次真实文生图，并把参考尝试、错误码和实际 `text_to_image` 模式写入记录；其他参考错误仍直接失败。只有两张最终模型输出及文件校验都成功，才把带评分、来源、提示词、模型、输入/输出 SHA-256 与生产边界的结果写入索引。高级研究链仍可越过 Designer Handoff 形成 DesignPackage、首样尺寸、BOM、装配、质检和展示海报。两条链都在 `production_release` 前停止。生产沟通图只用于概念拆解，不是开模图、CAD 或量产工程定稿；社区授权、工程验证、产品合规与商业发布仍需后续人工关口。
...
| 接口 | 作用 |
|---|---|
| `GET /api/workbench/bootstrap` | 一次取得工作区、22 条文化摘要、四个平台适配状态、启用/暂停清单、Top 10、人工决策目录与图像服务状态 |
...
每个组件都返回 `live`、`cache` 或 `unavailable` 状态；四个平台适配器还在 RunManifest 的 `market_platforms` 下按实际运行集合记录 `authorized|missing|expired`、登录/搜索是否成功和样本量。单个平台失败不会抹掉其他平台的独立结果，但严格晋级要求本轮启用集合精确匹配且全部为 `live`；暂停平台不运行、不阻断。缓存不会被写成实时数据，未披露的互动指标不会被推算。

市场 Adapter 仍只有一个：它保留 `xhs / dy / bili / wb` 四个适配器，但只依次运行 `MEDIACRAWLER_PLATFORMS` 中启用的平台；通用 Demo 默认保留四平台历史基线，当前 Zeabur 显式配置为 xhs/bili/wb，dy 暂停。各平台字段归一化为 MarketPost 后在平台内部计算百分位热度，再按产品形态汇总 Top 10 / Top 5。详细公式见 [`market_intelligence.md`](market_intelligence.md)。
