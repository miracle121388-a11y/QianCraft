...
系统每天把内容与形态进行受约束组合，选择评分最高且文化/形态互不重复的最多 3 个组合。每个结果必须由真实生图模型先生成设计效果图，再以该图为参考生成生产沟通图；两次调用全部成功后才保存。没有通过来源、样本或模型门槛的组合不会进入结果，也不会用兜底模板补足数量。
...
- 所有新设计都记录文化来源、代表市场记录、评分公式、两次模型调用、完整提示词、图生图输入摘要、版本和文件摘要。
- 两个持久调度线程分别负责持续采集和每日设计；健康接口同时检查线程与心跳。
- 原有九节点空间工作台保留在 `/workflow`，作为高级研究和旧式全链路操作入口。
- 0.11.0 的双模型视觉契约已完成本地实现与测试，等待本轮发布；受保护 Zeabur 暂时仍是 0.10.0 部署 `6a96ec9f40c09e36c3ebb590`。上一个三平台严格任务 `20260901T121642Z-e1a435ff` 的小红书 113、B站 110、微博 149 条 live 记录及 `live_verified` 工作区保持不变，抖音未进入清单。
