...
- Version 0.9.2 is accepted locally and on the protected Zeabur instance. Its three-OS GitHub CI, public-URL SSRF controls, API rate limiting, CSP/HSTS/security headers, non-root workers and verified runtime backup/restore all passed their actual release gates. The scheduler is not a distributed queue; strict live research remains blocked in the compact cloud image until its upstream runtimes and authorized platform sessions are supplied, and image regeneration remains unavailable until an independent image provider is configured.
